...
#import "WXNavigationProtocolImp.h"
...
+ (void)setup;
{
    ...
    [self initWeexSDK];
    [self loadCustomContainWithScannerWithUrl:[NSURL URLWithString:BUNDLE_URL]];
}
...
//    [WXSDKEngine registerHandler:[WXNavigationProtocolImp new] withProtocol:@protocol(WXNavigationProtocol)];
    ...
    [WXSDKEngine registerModule:@"hkImage" withClass:[HKImageModule class]];
    [WXSDKEngine registerModule:@"hkOrderProblemReporting" withClass:[WXDemoViewController class]];
...
//    HKWeexBaseViewController *vcl = [[HKWeexBaseViewController alloc]init];
//    vcl.url = url;
//    [[UIApplication sharedApplication] delegate].window.rootViewController = [[WXRootViewController alloc] initWithRootViewController:vcl];
}
