...
WX_EXPORT_METHOD(@selector(submit:photos:))

- (void)submit:(NSString *)feedback photos:(NSDictionary *)photos {
    NSLog(@"%@",feedback);
}
...
#if DEBUG
    NSString * hotReloadURL = @"http://192.168.31.211:8081/sockjs-node/541/zduezz3x/websocket";
    if (hotReloadURL){
...
//    [WXDevTool setDebug:YES];
    [WXDevTool launchDevToolDebugWithUrl:@"ws://192.168.31.211:8088/debugProxy/native"];
}
...
- (void)render
{
    ...
    [self.instance fireGlobalEvent:@"WXApplicationWillResignActiveEvent" params:@{@"test":@"test"}];
...
//    NSString *randomURL = [NSString stringWithFormat:@"%@%@random=%d",URL.absoluteString,URL.query?@"&":@"?",arc4random()];
    [_instance renderWithURL:self.url options:@{} data:nil];
}
